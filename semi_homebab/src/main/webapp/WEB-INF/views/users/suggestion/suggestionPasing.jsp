...
<body>
		...
		<c:forEach var="p" begin="${ requestScope.selectCriteria.startPage }" end="${ requestScope.selectCriteria.endPage }" step="1">
			...
			<c:if test="${ requestScope.selectCriteria.pageNo ne p }">
				<button onclick="pageButtonAction(this.innerText);" class="btn btn-default"><c:out value="${ p }"/></button>
			</c:if>
		</c:forEach>
	...
	<script>
		...
		const link = "${ pageContext.servletContext.contextPath }/suggestion/users/list";
		let searchText = "";
