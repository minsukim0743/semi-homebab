 ...
 */
@WebServlet("/freeboard/users/insert")
public class FreeboardInsertServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		request.getRequestDispatcher("/WEB-INF/views/users/freeboard/freeboardInsert.jsp").forward(request, response);
...
//	        fileUpload.setHeaderEncoding(encodingType);		//별 의미 없는 듯 하다. 기본값은 null인데 파일명은 자동으로 UTF-8로 인코딩한 거 같다.
	        ...
	        try {
				...
				System.out.println("syso = = = = = = =" + freeboard);
				
				List<FreeboardFileUploadDTO> list = new ArrayList<>();
				...
				System.out.println("syso = = = = = = =" + freeboard);
				System.out.println("freeboard freeboard : " + list);
				
				FreeboardService freeboardService = new FreeboardService();
				/* 서비스 메소드를 요청한다. */
				int result = freeboardService.freeboardInsert(freeboard);
				
				System.out.println("넌 뭐나오냐 제발 " + result);
				
				/* 성공 실패 페이지를 구분하여 연결한다. */
				String path = "";
				if(result > 0) {
					path = "/WEB-INF/views/common/success.jsp";
					request.setAttribute("successCode", "FreeboardInsert");
					request.setAttribute("insertFreeboard", result);
					
				} else {
					path = "/WEB-INF/views/common/failed.jsp";
					request.setAttribute("message", "자유 게시판 등록 실패!");
				}
